...
void* load_PE (char* PE_data) {
    ...
    IMAGE_SECTION_HEADER* section_headers = (IMAGE_SECTION_HEADER*) (p_NT_HDR + 1); 
    ...
    for(int i = 0; i < p_NT_HDR->FileHeader.NumberOfSections; ++i) {
        ...
        char* dest = ImageBase + section_headers[i].VirtualAddress; 
        ...
        if(section_headers[i].SizeOfRawData > 0) {
            ...
            memcpy(dest, PE_data + section_headers[i].PointerToRawData, section_headers[i].SizeOfRawData);
        } else {
            memset(dest, 0, section_headers[i].Misc.VirtualSize);
        }
    }
    ...
    for(int i = 0; i < p_NT_HDR->FileHeader.NumberOfSections; i++) {
        char* dest = ImageBase + section_headers[i].VirtualAddress;
        DWORD s_perm = section_headers[i].Characteristics;
        ...
        VirtualProtect(dest, section_headers[i].Misc.VirtualSize, v_perm, &oldProtect);
    }
